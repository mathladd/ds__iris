...
CURRENT_FILE_LINK = r'C:\Users\Alien\Downloads\datasets\dataset_auto\auto-mpg.csv'
...
L1_RATIO = 1  # Set this to 0 for linear ridge regression
...
def proc_convert_to_input(df, dependent):
    ...
    dependent_type = str(df[dependent].dtype)
    dependent_mapping = ''
...
def linear_regression_main(
        x, y, l1_ratio=1, penalty=0, cross_validation=1,
    ...
    if not cross_validation_check:
        ...
        cross_validated_mse = linear_regression_cv(x_with_const=x_with_const, y=y,
                                                   model='normal_fit', l1_ratio=l1_ratio, penalty=penalty,
    ...
    lasso_model = sm.OLS(y, x_with_const).fit_regularized(L1_wt=l1_ratio, alpha=penalty)
    if not cross_validation_check:
        cross_validated_mse = smte.mse(lasso_model.predict(x_with_const), y)
    else:
        cross_validated_mse = linear_regression_cv(x_with_const=x_with_const, y=y,
                                                   model='regularized', l1_ratio=l1_ratio, penalty=penalty,
                                                   cross_validation=cross_validation)
    ...
    if quadratic_test_linear:
        ...
        linear_for_quadratic_dict, linear_for_quadratic_dict_regularized = \
            linear_regression_quadratic_test(x=x, y=y, l1_ratio=L1_RATIO, penalty=PENALTY,
...
def linear_regression_cv(x_with_const, y, model, l1_ratio, penalty, cross_validation):
    ...
    if model == 'normal_fit' and loop_cross_val > 1:
        ...
        while loop_cross_val > 1:
            ...
            test_y = train_y.pop(loop_cross_val - 1)
            mse_mean += smte.mse(sm.OLS(pd.concat(train_y, axis=0), pd.concat(train_x, axis=0))
                                 .fit_regularized(L1_wt=l1_ratio, alpha=penalty).predict(test_x), test_y)
            loop_cross_val -= 1
        mse_mean = mse_mean / cross_validation
...
def linear_regression_quadratic_test(x, y, l1_ratio, penalty, cross_validation, quadratic_test_to_degree):
    mse_mean_list = []
    ...
    for degree in list_of_test_degree:
        ...
        mse_mean = linear_regression_cv(x_with_const=x_with_const, y=y, model='normal_fit',
                                        l1_ratio=l1_ratio, penalty=penalty, cross_validation=cross_validation)
